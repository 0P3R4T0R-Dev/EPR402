...
def makingImageThinned(filename, threshold=100, debug=False):
    image_ = readImage(filename)
    image_ = imageToBinary(image_, threshold=threshold)
    image_ = zhangSuenThinning(image_)
    if debug:
        displayImage(image_)
    saveImage(image_, filename[:-4] + " - thinned.jpg")


if __name__ == '__main__':
    image = readImage('Something is afoot and its not at the end of my leg.jpg')
    image = imageToBinary(image, threshold=100)
    displayImage(image)
    image = zhangSuenThinning(image)
    displayImage(image)
    saveImage(image, 'Something is afoot and its not at the end of my leg - thinned.jpg')
