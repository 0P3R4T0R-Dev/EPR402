...
def controlPenZ(serial_connection, up="UP"):
    if up == "UP":
        # print("Pen up")
...
def movePen(serial_connection, fromX, fromY, toX, toY):
...
def writeSentence(filename):
    ...
    while line != "Ready":
        ...
        time.sleep(0.001)
    print("Arduino: ", line)
    calibrate(serial_connection)
    startTime = time.time()
    image = readImage(filename)
    imArr = imageToBinary(image, threshold=100)
    ...
    controlPenZ(serial_connection, "DOWN")
    while pointer != (-1, -1):
        prevPointer = pointer
        pointer = nextPixel(imArr, pointer[0], pointer[1])
        if pointer != (-1, -1):
            movePen(serial_connection, prevPointer[0], prevPointer[1], pointer[0], pointer[1])
            canvas[pointer[0], pointer[1]] = 255
        else:
            controlPenZ(serial_connection, "UP")
